...
class Command(CommandBase):
    name = "wink"
    description = "Send a wink reaction to one or more users, or all users in the room. Usage: !wink @user1 @user2 ... [count] [interval=seconds] | !wink all [count] [interval=seconds]"
    aliases = []
    cooldown = 0
    permissions = []
    ...
    async def execute(self, user: User, args: list, message: str):
        ...
        if not targets:
            for _ in range(count):
                await self.bot.highrise.react("wink", user.id)
            ...
            if hasattr(users_resp, 'content'):
                for u, _ in users_resp.content:
                    if u.id != self.bot.highrise.my_id:
                        for _ in range(count):
                            await self.bot.highrise.react("wink", u.id)
                            await asyncio.sleep(interval)
            return
        users_resp = await self.bot.highrise.get_room_users()
        if hasattr(users_resp, 'content'):
            ...
            await self.bot.highrise.chat("Usage: !wink @user1 @user2 ... [count] [interval=seconds] | !wink all [count] [interval=seconds]")
