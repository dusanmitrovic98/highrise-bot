...
class Command(CommandBase):
    name = "heart"
    description = "Send a heart reaction to one or more users, or all users in the room. Usage: !heart @user1 @user2 ... [count] [interval=seconds] | !heart all [count] [interval=seconds]"
    aliases = []
    cooldown = 0
    permissions = []
    ...
    async def execute(self, user: User, args: list, message: str):
        count = 1
        interval = 0.3
        # Parse interval=number if present
            ...
            if hasattr(users_resp, 'content'):
                for u, _ in users_resp.content:
                    if u.id != self.bot.highrise.my_id:
                        for _ in range(count):
                            await self.bot.highrise.react("heart", u.id)
                            await asyncio.sleep(interval)
            return
        users_resp = await self.bot.highrise.get_room_users()
        if hasattr(users_resp, 'content'):
            ...
            await self.bot.highrise.chat("Usage: !heart @user1 @user2 ... [count] [interval=seconds] | !heart all [count] [interval=seconds]")
